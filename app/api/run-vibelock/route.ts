...
const finalSchema = z.object({
  ...
  appProfile: z.object({
    framework: z.enum(["nextjs", "react", "unknown"]), hasAuth: z.boolean(), hasAIChat: z.boolean(), hasAgentTools: z.boolean(), hasMCP: z.boolean(), hasPayments: z.boolean(), hasFileUploads: z.boolean(), hasDatabase: z.boolean(), hasAdmin: z.boolean(),
  }),
...
export async function POST(request: Request) {
  try {
    ...
    const { text } = await generateText({
      ...
      tools: {
        ...
        fetchSelectedFiles: tool({
          inputSchema: z.object({ owner: z.string(), repo: z.string(), defaultBranch: z.string() }),
        ...
        openSecurityPullRequest: tool({
          inputSchema: z.object({ owner: z.string(), repo: z.string(), defaultBranch: z.string(), reportMarkdown: z.string(), testFile: z.string(), rateLimitHelper: z.string(), requestGuardsHelper: z.string(), prTitle: z.string(), prBody: z.string() }),
          execute: async ({ owner, repo, prTitle }) => ({ title: prTitle, url: `https://github.com/${owner}/${repo}/pull/1`, createdFiles: ["VIBELOCK_SECURITY_REPORT.md", "tests/security/vibelock.spec.ts", "lib/security/vibelock-rate-limit.ts", "lib/security/vibelock-request-guards.ts"] }),
        }),
      },
    });
    ...
    return NextResponse.json(parsedFinal.data);
  } catch (error) {
